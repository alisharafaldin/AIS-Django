...
def invoices_sales_search(request):
   ...
   # الحصول على فواتير المبيعات الخاصة بالشركة الحالية
    ...
    total_sum = invoices.aggregate(
        total_sum=Sum('sales_invoice__total_price_after_tax'))['total_sum'] or 0

    # total_sum = total_invoices_sum['total_sum'] or 0

    # إجمالي الفواتير بالعملة المحلية
    total_local_currency = invoices.aggregate(
        total_sum=Sum('sales_invoice__total_price_local_currency'))['total_sum'] or 0
...
def sold_products(request):
    ...
    # الحصول على فواتير المبيعات الخاصة بالشركة الحالية
    invoices = sold_products.filter(invoiceHeadID__companyID_id=current_company_id)
