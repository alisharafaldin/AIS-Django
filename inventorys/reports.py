...
def item_quantity_summary(request):
    ...
    context = {
        ...
        'search_form': SetComboBox(request.GET),  # تمرير معايير البحث الحالية
        'ordering': ordering.strip('-'),  # تمرير ترتيب الحقل الحالي إلى القالب
        'direction': direction,  # تمرير اتجاه الترتيب الحالي إلى القالب
    }
    
    return render(request, 'inventorys/item_quantity_summary.html', context)
